...
public class ProjectStatsService {
    ...
    public List<ProjectStats> fetchProjectsStatistics() {
        ...
        List<ProjectStats> projectStats = projects.stream().map(project -> {
            ...
            Integer estimatedEfforts = project.getTasks().stream().map(Task::getEstimation).reduce(0, Integer::sum);
    ...
    public Integer getActualEfforts(UUID projectId) {
        return dataManager.loadValue(
                        "select sum(te.timeSpent) from TimeEntry te where te.task.project.id = :projectId", Integer.class)
